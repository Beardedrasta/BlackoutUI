--------------------------------------------------
-- BLACKOUT UI
-- Modules/TargetFrame.lua
--------------------------------------------------

local addonName, BlackoutUI = ...

--------------------------------------------------
-- CONFIGURATION
--------------------------------------------------

local WIDTH = 360
local HEIGHT = 99

local HEADER_HEIGHT = 22
local HEALTH_HEIGHT = 43
local POWER_HEIGHT = 19

--------------------------------------------------
-- MAIN TARGET FRAME
--------------------------------------------------

local TargetFrame = CreateFrame(
    "Button",
    "BlackoutUI_TargetFrame",
    UIParent,
    "SecureUnitButtonTemplate"
)

TargetFrame:SetSize(WIDTH, HEIGHT)

TargetFrame:SetPoint(
    "CENTER",
    UIParent,
    "CENTER",
    260,
    -180
)

TargetFrame:SetFrameStrata("MEDIUM")

--------------------------------------------------
-- UNIT BEHAVIOR
--------------------------------------------------

TargetFrame:SetAttribute(
    "unit",
    "target"
)

TargetFrame:RegisterForClicks(
    "AnyUp"
)

TargetFrame:SetAttribute(
    "*type1",
    "target"
)

TargetFrame:SetAttribute(
    "*type2",
    "togglemenu"
)

--------------------------------------------------
-- MAIN BACKGROUND
--------------------------------------------------

local MainBackground =
    TargetFrame:CreateTexture(
        nil,
        "BACKGROUND"
    )

MainBackground:SetAllPoints()

MainBackground:SetColorTexture(
    0.018,
    0.018,
    0.018,
    0.97
)

--------------------------------------------------
-- OUTER BORDER
--------------------------------------------------

BlackoutUI:CreateBorder(
    TargetFrame,
    1,
    "borderBright"
)

--------------------------------------------------
-- INNER FRAME
--------------------------------------------------

local InnerFrame = CreateFrame(
    "Frame",
    nil,
    TargetFrame
)

InnerFrame:SetPoint(
    "TOPLEFT",
    2,
    -2
)

InnerFrame:SetPoint(
    "BOTTOMRIGHT",
    -2,
    2
)

BlackoutUI:CreateBorder(
    InnerFrame,
    1,
    "border"
)

--------------------------------------------------
-- HEADER
--------------------------------------------------

local Header = CreateFrame(
    "Frame",
    nil,
    InnerFrame
)

Header:SetPoint(
    "TOPLEFT",
    3,
    -3
)

Header:SetPoint(
    "TOPRIGHT",
    -3,
    -3
)

Header:SetHeight(
    HEADER_HEIGHT
)

--------------------------------------------------
-- HEADER BACKGROUND
--------------------------------------------------

local HeaderBackground =
    Header:CreateTexture(
        nil,
        "BACKGROUND"
    )

HeaderBackground:SetAllPoints()

HeaderBackground:SetColorTexture(
    0.045,
    0.045,
    0.045,
    1
)

--------------------------------------------------
-- TARGET ACCENT
--------------------------------------------------

local TargetAccent =
    Header:CreateTexture(
        nil,
        "ARTWORK"
    )

TargetAccent:SetPoint(
    "TOPRIGHT",
    Header,
    "TOPRIGHT",
    0,
    0
)

TargetAccent:SetPoint(
    "BOTTOMRIGHT",
    Header,
    "BOTTOMRIGHT",
    0,
    0
)

TargetAccent:SetWidth(3)

TargetAccent:SetColorTexture(
    1,
    1,
    1,
    1
)

--------------------------------------------------
-- TARGET NAME
--------------------------------------------------

local NameText =
    BlackoutUI:CreateFont(
        Header,
        12
    )

NameText:SetPoint(
    "RIGHT",
    TargetAccent,
    "LEFT",
    -7,
    0
)

NameText:SetJustifyH(
    "RIGHT"
)

--------------------------------------------------
-- LEVEL / CLASSIFICATION
--------------------------------------------------

local LevelText =
    BlackoutUI:CreateFont(
        Header,
        10
    )

LevelText:SetPoint(
    "LEFT",
    Header,
    "LEFT",
    6,
    0
)

LevelText:SetTextColor(
    0.60,
    0.60,
    0.60
)

--------------------------------------------------
-- HEALTH CONTAINER
--------------------------------------------------

local HealthContainer =
    CreateFrame(
        "Frame",
        nil,
        InnerFrame
    )

HealthContainer:SetPoint(
    "TOPLEFT",
    Header,
    "BOTTOMLEFT",
    0,
    -3
)

HealthContainer:SetPoint(
    "TOPRIGHT",
    Header,
    "BOTTOMRIGHT",
    0,
    -3
)

HealthContainer:SetHeight(
    HEALTH_HEIGHT
)

--------------------------------------------------
-- HEALTH BACKGROUND
--------------------------------------------------

local HealthBackground =
    HealthContainer:CreateTexture(
        nil,
        "BACKGROUND"
    )

HealthBackground:SetAllPoints()

HealthBackground:SetColorTexture(
    0.055,
    0.055,
    0.055,
    1
)

--------------------------------------------------
-- HEALTH BAR
--------------------------------------------------

local HealthBar =
    CreateFrame(
        "StatusBar",
        nil,
        HealthContainer
    )

HealthBar:SetPoint(
    "TOPLEFT",
    1,
    -1
)

HealthBar:SetPoint(
    "BOTTOMRIGHT",
    -1,
    1
)

HealthBar:SetStatusBarTexture(
    "Interface\\Buttons\\WHITE8X8"
)

HealthBar:SetMinMaxValues(
    0,
    100
)

HealthBar:SetValue(
    100
)

--------------------------------------------------
-- HEALTH LOWER SHADE
--------------------------------------------------

local HealthShade =
    HealthBar:CreateTexture(
        nil,
        "ARTWORK"
    )

HealthShade:SetPoint(
    "BOTTOMLEFT"
)

HealthShade:SetPoint(
    "BOTTOMRIGHT"
)

HealthShade:SetHeight(
    HEALTH_HEIGHT * 0.45
)

HealthShade:SetColorTexture(
    0,
    0,
    0,
    0.12
)

--------------------------------------------------
-- TOP HIGHLIGHT
--------------------------------------------------

local HealthHighlight =
    HealthBar:CreateTexture(
        nil,
        "ARTWORK"
    )

HealthHighlight:SetPoint(
    "TOPLEFT"
)

HealthHighlight:SetPoint(
    "TOPRIGHT"
)

HealthHighlight:SetHeight(1)

HealthHighlight:SetColorTexture(
    1,
    1,
    1,
    0.10
)

--------------------------------------------------
-- HEALTH VALUE
--------------------------------------------------

local HealthValue =
    BlackoutUI:CreateFont(
        HealthBar,
        12
    )

HealthValue:SetPoint(
    "RIGHT",
    HealthBar,
    "RIGHT",
    -8,
    0
)

--------------------------------------------------
-- HEALTH PERCENT
--------------------------------------------------

local HealthPercent =
    BlackoutUI:CreateFont(
        HealthBar,
        14
    )

HealthPercent:SetPoint(
    "LEFT",
    HealthBar,
    "LEFT",
    8,
    0
)

--------------------------------------------------
-- POWER CONTAINER
--------------------------------------------------

local PowerContainer =
    CreateFrame(
        "Frame",
        nil,
        InnerFrame
    )

PowerContainer:SetPoint(
    "TOPLEFT",
    HealthContainer,
    "BOTTOMLEFT",
    0,
    -3
)

PowerContainer:SetPoint(
    "TOPRIGHT",
    HealthContainer,
    "BOTTOMRIGHT",
    0,
    -3
)

PowerContainer:SetHeight(
    POWER_HEIGHT
)

--------------------------------------------------
-- POWER BACKGROUND
--------------------------------------------------

local PowerBackground =
    PowerContainer:CreateTexture(
        nil,
        "BACKGROUND"
    )

PowerBackground:SetAllPoints()

PowerBackground:SetColorTexture(
    0.04,
    0.04,
    0.04,
    1
)

--------------------------------------------------
-- POWER BAR
--------------------------------------------------

local PowerBar =
    CreateFrame(
        "StatusBar",
        nil,
        PowerContainer
    )

PowerBar:SetPoint(
    "TOPLEFT",
    1,
    -1
)

PowerBar:SetPoint(
    "BOTTOMRIGHT",
    -1,
    1
)

PowerBar:SetStatusBarTexture(
    "Interface\\Buttons\\WHITE8X8"
)

PowerBar:SetMinMaxValues(
    0,
    100
)

PowerBar:SetValue(
    100
)

--------------------------------------------------
-- POWER LABEL
--------------------------------------------------

local PowerLabel =
    BlackoutUI:CreateFont(
        PowerBar,
        9
    )

PowerLabel:SetPoint(
    "RIGHT",
    PowerBar,
    "RIGHT",
    -6,
    0
)

--------------------------------------------------
-- POWER VALUE
--------------------------------------------------

local PowerValue =
    BlackoutUI:CreateFont(
        PowerBar,
        10
    )

PowerValue:SetPoint(
    "LEFT",
    PowerBar,
    "LEFT",
    6,
    0
)

--------------------------------------------------
-- UPDATE TARGET NAME
--------------------------------------------------

local function UpdateName()
    if not UnitExists("target") then
        return
    end

    local name =
        UnitName("target")
        or "Unknown"

    NameText:SetText(
        string.upper(name)
    )
end

--------------------------------------------------
-- UPDATE TARGET LEVEL
--------------------------------------------------

local function UpdateLevel()
    if not UnitExists("target") then
        return
    end

    local level =
        UnitLevel("target")

    local classification =
        UnitClassification("target")

    local levelString

    --------------------------------------------------
    -- BOSS / UNKNOWN LEVEL
    --------------------------------------------------

    if level == -1 then
        levelString = "BOSS"
    else
        levelString =
            "LVL " .. level
    end

    --------------------------------------------------
    -- CLASSIFICATION
    --------------------------------------------------

    if classification == "elite" then
        levelString =
            levelString .. " • ELITE"
    elseif classification == "rare" then
        levelString =
            levelString .. " • RARE"
    elseif classification == "rareelite" then
        levelString =
            levelString .. " • RARE ELITE"
    elseif classification == "worldboss" then
        levelString =
        "BOSS"
    end

    LevelText:SetText(
        levelString
    )
end

--------------------------------------------------
-- UPDATE REACTION / TARGET COLOR
--------------------------------------------------

local function UpdateTargetColor()
    if not UnitExists("target") then
        return
    end

    local r
    local g
    local b

    --------------------------------------------------
    -- PLAYER
    --------------------------------------------------

    if UnitIsPlayer("target") then
        local _, classFile =
            UnitClass("target")

        local classColor =
            classFile
            and RAID_CLASS_COLORS[classFile]

        if classColor then
            r = classColor.r
            g = classColor.g
            b = classColor.b
        end
    end

    --------------------------------------------------
    -- NPC / FALLBACK
    --------------------------------------------------

    if not r then
        local reaction =
            UnitReaction(
                "target",
                "player"
            )

        --------------------------------------------------
        -- HOSTILE
        --------------------------------------------------

        if reaction
            and reaction <= 3 then
            r = 0.78
            g = 0.14
            b = 0.14

            --------------------------------------------------
            -- NEUTRAL
            --------------------------------------------------
        elseif reaction == 4 then
            r = 0.82
            g = 0.68
            b = 0.08

            --------------------------------------------------
            -- FRIENDLY
            --------------------------------------------------
        elseif reaction
            and reaction >= 5 then
            r = 0.18
            g = 0.68
            b = 0.28
        else
            r = 0.50
            g = 0.50
            b = 0.50
        end
    end

    --------------------------------------------------
    -- APPLY COLOR
    --------------------------------------------------

    TargetAccent:SetColorTexture(
        r,
        g,
        b,
        1
    )

    NameText:SetTextColor(
        r,
        g,
        b
    )
end

--------------------------------------------------
-- UPDATE HEALTH
--------------------------------------------------

local function UpdateHealth()
    if not UnitExists("target") then
        return
    end

    local health =
        UnitHealth("target")

    local maxHealth =
        UnitHealthMax("target")

    if maxHealth <= 0 then
        maxHealth = 1
    end

    --------------------------------------------------
    -- BAR
    --------------------------------------------------

    HealthBar:SetMinMaxValues(
        0,
        maxHealth
    )

    HealthBar:SetValue(
        health
    )

    --------------------------------------------------
    -- DEAD
    --------------------------------------------------

    if UnitIsDeadOrGhost("target") then
        HealthBar:SetStatusBarColor(
            0.24,
            0.24,
            0.24,
            1
        )

        HealthValue:SetText(
            "DEAD"
        )

        HealthPercent:SetText("")

        return
    end

    --------------------------------------------------
    -- HEALTH %
    --------------------------------------------------

    local percentage =
        health / maxHealth

    --------------------------------------------------
    -- HEALTH COLOR
    --------------------------------------------------

    local r
    local g
    local b

    if percentage > 0.50 then
        r = 0.16
        g = 0.62
        b = 0.25
    elseif percentage > 0.25 then
        r = 0.80
        g = 0.55
        b = 0.10
    else
        r = 0.78
        g = 0.12
        b = 0.12
    end

    HealthBar:SetStatusBarColor(
        r,
        g,
        b,
        1
    )

    --------------------------------------------------
    -- HEALTH TEXT
    --------------------------------------------------

    HealthValue:SetText(
        BlackoutUI:FormatNumber(health)
        ..
        " / "
        ..
        BlackoutUI:FormatNumber(maxHealth)
    )

    HealthPercent:SetText(
        string.format(
            "%d%%",
            percentage * 100
        )
    )
end

--------------------------------------------------
-- UPDATE POWER
--------------------------------------------------

local function UpdatePower()
    if not UnitExists("target") then
        return
    end

    local powerType,
    powerToken =
        UnitPowerType("target")

    local power =
        UnitPower(
            "target",
            powerType
        )

    local maxPower =
        UnitPowerMax(
            "target",
            powerType
        )

    --------------------------------------------------
    -- TARGET HAS NO POWER
    --------------------------------------------------

    if not maxPower
        or maxPower <= 0 then
        PowerBar:SetMinMaxValues(
            0,
            1
        )

        PowerBar:SetValue(0)

        PowerLabel:SetText("")

        PowerValue:SetText("")

        PowerBar:SetStatusBarColor(
            0.20,
            0.20,
            0.20,
            1
        )

        return
    end

    --------------------------------------------------
    -- BAR
    --------------------------------------------------

    PowerBar:SetMinMaxValues(
        0,
        maxPower
    )

    PowerBar:SetValue(
        power
    )

    --------------------------------------------------
    -- MANA
    --------------------------------------------------

    if powerType == 0 then
        PowerBar:SetStatusBarColor(
            0.10,
            0.35,
            0.82,
            1
        )

        PowerLabel:SetText(
            "MANA"
        )

        --------------------------------------------------
        -- RAGE
        --------------------------------------------------
    elseif powerType == 1 then
        PowerBar:SetStatusBarColor(
            0.75,
            0.10,
            0.10,
            1
        )

        PowerLabel:SetText(
            "RAGE"
        )

        --------------------------------------------------
        -- ENERGY
        --------------------------------------------------
    elseif powerType == 3 then
        PowerBar:SetStatusBarColor(
            0.82,
            0.68,
            0.08,
            1
        )

        PowerLabel:SetText(
            "ENERGY"
        )

        --------------------------------------------------
        -- OTHER
        --------------------------------------------------
    else
        PowerBar:SetStatusBarColor(
            0.40,
            0.40,
            0.40,
            1
        )

        PowerLabel:SetText(
            powerToken or "POWER"
        )
    end

    PowerValue:SetText(
        BlackoutUI:FormatNumber(power)
        ..
        " / "
        ..
        BlackoutUI:FormatNumber(maxPower)
    )
end

--------------------------------------------------
-- UPDATE EVERYTHING
--------------------------------------------------

local function UpdateAll()
    if not UnitExists("target") then
        TargetFrame:Hide()

        return
    end

    TargetFrame:Show()

    UpdateName()
    UpdateLevel()
    UpdateTargetColor()
    UpdateHealth()
    UpdatePower()
end

--------------------------------------------------
-- EVENTS
--------------------------------------------------

TargetFrame:RegisterEvent(
    "PLAYER_ENTERING_WORLD"
)

TargetFrame:RegisterEvent(
    "PLAYER_TARGET_CHANGED"
)

TargetFrame:RegisterEvent(
    "UNIT_HEALTH"
)

TargetFrame:RegisterEvent(
    "UNIT_MAXHEALTH"
)

TargetFrame:RegisterEvent(
    "UNIT_POWER_UPDATE"
)

TargetFrame:RegisterEvent(
    "UNIT_MAXPOWER"
)

TargetFrame:RegisterEvent(
    "UNIT_DISPLAYPOWER"
)

TargetFrame:RegisterEvent(
    "UNIT_FACTION"
)

--------------------------------------------------
-- EVENT HANDLER
--------------------------------------------------

TargetFrame:SetScript(
    "OnEvent",
    function(self, event, unit)
        --------------------------------------------------
        -- WORLD / TARGET CHANGE
        --------------------------------------------------

        if event == "PLAYER_ENTERING_WORLD"
            or event == "PLAYER_TARGET_CHANGED" then
            UpdateAll()

            return
        end

        --------------------------------------------------
        -- ONLY CARE ABOUT TARGET
        --------------------------------------------------

        if unit
            and unit ~= "target" then
            return
        end

        --------------------------------------------------
        -- HEALTH
        --------------------------------------------------

        if event == "UNIT_HEALTH"
            or event == "UNIT_MAXHEALTH" then
            UpdateHealth()

            --------------------------------------------------
            -- POWER
            --------------------------------------------------
        elseif event == "UNIT_POWER_UPDATE"
            or event == "UNIT_MAXPOWER"
            or event == "UNIT_DISPLAYPOWER" then
            UpdatePower()

            --------------------------------------------------
            -- REACTION
            --------------------------------------------------
        elseif event == "UNIT_FACTION" then
            UpdateTargetColor()
        end
    end
)


--------------------------------------------------
-- CONFIG API
--------------------------------------------------

BlackoutUI.TargetFrame = BlackoutUI.TargetFrame or {}

local function GetTargetConfig()
    BlackoutUIDB = BlackoutUIDB or {}
    BlackoutUIDB.Config = BlackoutUIDB.Config or {}
    BlackoutUIDB.Config.UnitFrames = BlackoutUIDB.Config.UnitFrames or {}
    BlackoutUIDB.Config.UnitFrames.target = BlackoutUIDB.Config.UnitFrames.target or {}

    local c = BlackoutUIDB.Config.UnitFrames.target
    if c.enabled == nil then c.enabled = true end
    if c.width == nil then c.width = 360 end
    if c.height == nil then c.height = 99 end
    if c.scale == nil then c.scale = 1.00 end
    if c.headerHeight == nil then c.headerHeight = 22 end
    if c.healthHeight == nil then c.healthHeight = 43 end
    if c.powerHeight == nil then c.powerHeight = 19 end
    if c.nameSize == nil then c.nameSize = 12 end
    if c.levelSize == nil then c.levelSize = 10 end
    if c.healthValueSize == nil then c.healthValueSize = 12 end
    if c.healthPercentSize == nil then c.healthPercentSize = 14 end
    if c.powerLabelSize == nil then c.powerLabelSize = 9 end
    if c.powerValueSize == nil then c.powerValueSize = 10 end
    if c.showName == nil then c.showName = true end
    if c.showLevel == nil then c.showLevel = true end
    if c.showHealthValue == nil then c.showHealthValue = true end
    if c.showHealthPercent == nil then c.showHealthPercent = true end
    if c.showPowerLabel == nil then c.showPowerLabel = true end
    if c.showPowerValue == nil then c.showPowerValue = true end
    return c
end

local function SetFontSize(fontString, size)
    local font, _, flags = fontString:GetFont()
    if font then fontString:SetFont(font, size, flags) end
end

function BlackoutUI.TargetFrame:ApplyConfig()
    if InCombatLockdown() then return end

    local c = GetTargetConfig()

    TargetFrame:SetSize(c.width, c.height)
    TargetFrame:SetScale(c.scale)

    Header:SetHeight(c.headerHeight)
    HealthContainer:SetHeight(c.healthHeight)
    PowerContainer:SetHeight(c.powerHeight)
    HealthShade:SetHeight(c.healthHeight * 0.45)

    SetFontSize(NameText, c.nameSize)
    SetFontSize(LevelText, c.levelSize)
    SetFontSize(HealthValue, c.healthValueSize)
    SetFontSize(HealthPercent, c.healthPercentSize)
    SetFontSize(PowerLabel, c.powerLabelSize)
    SetFontSize(PowerValue, c.powerValueSize)

    NameText:SetShown(c.showName)
    LevelText:SetShown(c.showLevel)
    HealthValue:SetShown(c.showHealthValue)
    HealthPercent:SetShown(c.showHealthPercent)
    PowerLabel:SetShown(c.showPowerLabel)
    PowerValue:SetShown(c.showPowerValue)

    if not c.enabled then
        TargetFrame:Hide()
    else
        UpdateAll()
    end
end

function BlackoutUI.TargetFrame:ResetConfig()
    if InCombatLockdown() then return end
    BlackoutUIDB.Config.UnitFrames.target = nil
    GetTargetConfig()
    self:ApplyConfig()
end

--------------------------------------------------
-- BLACKOUT MOVER
--------------------------------------------------

BlackoutUI:RegisterMovableFrame(
    TargetFrame,
    "TargetFrame",
    "TARGET FRAME"
)


--------------------------------------------------
-- START HIDDEN
--------------------------------------------------

TargetFrame:Hide()

BlackoutUI.TargetFrame:ApplyConfig()
